...
export default function WallHeatmap({ climb_ids }: { climb_ids: number[] }) {
    ...
    return (
        <div className="flex flex-col items-center gap-2 w-full overflow-clip">
            <p className="font-bold text-neutral-800 text-lg md:text-xl">
                Walls
            </p>
            <div className="max-w-[500px]">
                <div className="relative w-[500px] scale-[0.7] sm:scale-[0.85] md:scale-100 origin-top">
                    <WallMap className="fill-slate-300 stroke-neutral-800 border-3 border-neutral-800 w-full h-auto" />
                    {data.x_labels.map((label, index) => {
                        if (label == Wall.other) return null;
                        return (
                            <div key={index}>
                                <p
                                    className={`absolute font-semibold text-black text-xs md:text-sm text-center ${labelToPositions[label as Wall][0]}`}
                                >
                                    {label === Wall.bigShow ? (
                                        <span>
                                            Big <br /> Show
                                ...
                                {data.y_values[index] > 0 && (
                                    <div
                                        ...
                                        className={`absolute rounded-full w-16 md:w-20 -z-10 aspect-square ${labelToPositions[label as Wall][1]}`}
                                    />
                                )}
                            </div>
                        );
